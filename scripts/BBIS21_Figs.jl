...
function FigureTwoBalls(B1::EuclideanBall, 
    ...
    MethodPath!(plt, [z₀' ; zRB1'; zRB2'; z₀' ], arrow=:none, color =:black, lalpha = 0.2, ls=:dot)
    MethodPath!(plt, [z₀' ; zpCRM' ], color = :black, lalpha=0.5)
    plot!(Ball2(zpCRM,norm(zpCRM-z₀)),alpha = 0.,ls=:dot,lalpha=0.2)
...
annotate!([(0., 1.5, text(L"X",labelsize))])
...
xlims!(plt2, 0, 1., aspect_ratio=:equal)
...
plot!(Ball2(zC_CRM,norm(zC_CRM-zCent)),alpha = 0.,ls=:dot,lalpha=0.2)
MethodPath!(plt2, [zCent'; zC_CRM' ], color = :black, lalpha = 0.5,λ = 0.88)
MethodPath!(plt2, [zCent' ; zRB1'; zRB2'; zCent' ], arrow=:none, color =:black, lalpha = 0.2, ls=:dot)
v₁ = zCent - zPB1
b₁ = dot(v₁,zPB1)
